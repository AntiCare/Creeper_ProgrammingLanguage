...
public class Compiler {
	...
	private ParseTreeProperty<DataType> dataTypes = new ParseTreeProperty<>();
	...
	private ParseTree runLexerAndParser( CharStream input ) {
		GrammarLexer lexer = new GrammarLexer(input);      // TODO: Replace lexer with the one for your own language
		lexer.addErrorListener(getErrorListener());
		CommonTokenStream tokens = new CommonTokenStream(lexer);

		GrammarParser parser = new GrammarParser(tokens);  // TODO: Replace parser with the one for your own language
	...
	private boolean runChecker( ParseTree parseTree ) {
		...
		Checker checker = new Checker(dataTypes,scope, symbolTable);
	...
	private JasminBytecode generateCode( ParseTree parseTree, String className ) {
        ...
        Visitor vistor =new Visitor(dataTypes, scope);
        ArrayList<String> code = vistor.visit(parseTree);

		jasminBytecode.add(".method public static main([Ljava/lang/String;)V")
				.add(".limit stack 999")
				.add(".limit locals 999")  // NOTE: The args-parameter is a local too
				.add()
				.addAll(code)
				.add("return")
				.add(".end method");
		if(Visitor.haveMethod){
			for (int i = 0; i < Visitor.method.size(); i++) {
				if(i%2==0){
					jasminBytecode.addAll(Visitor.method.get(i));
				}
